<div class="overflow-hidden bg-white shadow-md rounded-lg">
    <div class="p-4 border-b border-gray-200">
        <div class="relative">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" class="block w-full py-2 pl-10 pr-3 text-sm border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="Search conversations">
        </div>
    </div>
    <ul class="divide-y divide-gray-200 max-h-[70vh] overflow-y-auto">
        @forelse($conversations as $conversation)
            ...
            @endphp
            <li class="cursor-pointer {{ $isSelected ? 'bg-blue-50' : 'hover:bg-gray-50' }}"
                    ...
                    <div class="flex-1 min-w-0 ml-4">
                        ...
                        <div class="flex items-center justify-between mt-1">
                            <p class="text-xs text-gray-500 truncate">
                                {{ $conversation->last_message_text ? (strlen($conversation->last_message_text) > 30 ? substr($conversation->last_message_text, 0, 30).'...' : $conversation->last_message_text) : 'No messages yet' }}
                            </p>
                            @if($unreadCount > 0)
