...
class EnhancedOrderService implements EnhancedOrderServiceInterface
{
    ...
    public function rejectOrder(int $orderId, int $approverId, string $reason = null): Order
    {
        ...
        try {
            ...
            Log::info("Order rejected", ['order_id' => $orderId, 'approver_id' => $approverId, 'reason' => $reason]);
            
            return $order->fresh(['buyer', 'seller']);
            
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error("Order rejection failed", ['order_id' => $orderId, 'error' => $e->getMessage()]);
            throw $e;
        }
    }
    ...
    public function calculateOrderValue(array $items): float
    {
        ...
        foreach ($items as $item) {
            ...
            if ($resource) {
                $totalValue += $resource->price * $item['quantity'];
            }
        }
    ...
    public function checkInventoryAvailability(array $items): array
    {
        ...
        foreach ($items as $item) {
            $stock = $this->inventoryService->getAvailableStock($item['resource_id']);
            
            if ($stock < $item['quantity']) {
                $available = false;
                $resource = Resource::find($item['resource_id']);
                $unavailableItems[] = $resource ? $resource->name : "Resource ID {$item['resource_id']}";
            }
        }
